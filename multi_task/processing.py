...
class MultiTaskDatasetByQuery():
    ...
    def build_dataset_by_query_entity_context_news(self, dir_path='/nfs/trec_news_track/data/5_fold/', max_rank=100, batch_size=64, passage_model_path=None,
        ...
        for dataset in ['valid', 'test', 'train']:
            ...
            passage_to_entity_path = fold_dir_path + 'doc_to_entity_map.json'
            with open(passage_to_entity_path, 'r') as f:
                passage_to_entity_dict = json.load(f)
            ...
            for query_i, query in enumerate(queries):
                ...
                # if ((query_i > 15) and  (dataset == 'valid')) or (dataset in ['test', 'train']):
                entity_context_dataset = {}
                self.cls_id = 0
                self.cls_id_list = []
                self.token_list = []
                ...
                for run_data in passage_run_dict[query]:
                    ...
                    if len(entity_links) == 0:
                        ...
                        try:
                            ...
                            try:
                                entity_links = [search_tools_entity.search(query=passage_text[:100], hits=1)[0][0]]
                            except:
                                entity_links = [search_tools_entity.search(query='random entity', hits=1)[0][0]]
                        print('Synthetic entity links:', entity_links)
    ...
    def build_dataset_by_query_entity_context_news_keyword(self, dir_path='/nfs/trec_news_track/data/5_fold/', max_rank=100,
                                                   batch_size=64, passage_model_path=None,
                                                   entity_model_path=None, query_keyword_path=None, doc_keyword_path=None):
        """ """

        passage_model = BertMultiTaskRanker.from_pretrained(passage_model_path)
        entity_model = BertMultiTaskRanker.from_pretrained(entity_model_path)

        with open(query_keyword_path, 'r') as f:
            query_keyword_dict = json.load(f)

        with open(doc_keyword_path, 'r') as f:
            doc_keyword_dict = json.load(f)
        ...
        for dataset in ['valid', 'test', 'train']:
            ...
            for query_i, query in enumerate(queries):
                ...
                # Update BERT cls input
                try:
                    query_decoded = query_keyword_dict[query]['query_100_words']
                except:
                    print(query_i, 'missing from query_keyword_dict')
                    query_dict = json.loads(search_tools_passage.get_contents_from_docid(query))
                    query_decoded = search_tools_passage.process_query_news(query_dict=query_dict, query_type='title+contents')
                ...
                for run_data in passage_run_dict[query]:
                    ...
                    # --- Add passage ---
                    try:
                        passage_text = doc_keyword_dict[passage_id]['query_100_words']
                    except:
                        print(passage_id, 'missing from doc_keyword_dict')
                        passage_dict = json.loads(search_tools_passage.get_contents_from_docid(passage_id))
                        passage_text = search_tools_passage.process_query_news(query_dict=passage_dict,
                                                                               query_type='title+contents')
                    passage_input_ids = tokenizer.encode(text=query_decoded,
                ...
                print('--- TO JSON ---')
                mutant_data_folder = fold_dir_path + '/mutant_keyword_data/'
                if os.path.isdir(mutant_data_folder) == False:
                    os.mkdir(mutant_data_folder)
                ...
                query_json_path = mutant_data_folder_dataset + '{}_mutant_max.json'.format(query_i)
                with open(query_json_path, 'w') as f:
                    json.dump(entity_context_dataset, f)

        query_id_to_query_json_path = {}
        for dataset in ['valid', 'test', 'train']:
            file_name = '_mutant_max.json'
            mutant_data_folder_dataset = dir_path + 'scaled_5fold_{}_data/'.format(fold) + '/mutant_keyword_data/' + dataset + '/'
            query_json_paths = [mutant_data_folder_dataset + f for f in os.listdir(mutant_data_folder_dataset) if file_name in f]
            for path in query_json_paths:
                with open(path, 'r') as f:
                    d = json.load(f)
                    query_id_to_query_json_path[d['query']['query_id']] = path
        ...
        for fold in [1,2,3,4]:
            for dataset in ['valid', 'test', 'train']:
                ...
                for query_i, query in enumerate(queries):
                    src = query_id_to_query_json_path[query]
                    mutant_data_folder = fold_dir_path + '/mutant_keyword_data/'
                    if os.path.isdir(mutant_data_folder) == False:
                        os.mkdir(mutant_data_folder)
                    mutant_data_folder_dataset = mutant_data_folder + dataset + '/'
                    if os.path.isdir(mutant_data_folder_dataset) == False:
                        os.mkdir(mutant_data_folder_dataset)
                    dst = mutant_data_folder_dataset + '{}_mutant_max.json'.format(query_i)
