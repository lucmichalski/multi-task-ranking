...
def train_cls_model_max_combo(batch_size=256, lr=0.0005, parent_dir_path='/nfs/trec_car/data/entity_ranking/multi_task_data_by_query/'):
    """ """
    train_dir_path = parent_dir_path + 'train_data/'
    dev_dir_path = parent_dir_path + 'dev_data/'
    test_dir_path = parent_dir_path + 'test_data/'
    ...
    for task in ['passage']:
        ...
        for label, score, run_data in zip(test_label, test_score, test_run_data):
            ...
            if (topic_query != None) and (topic_query != query):
                ...
                assert len(topic_run_data) <= 100, "{} len {}: {}".format(topic_query, len(topic_run_data), [x for x in [i[0] for i in topic_run] if [i[0] for i in topic_run].count(x) > 1]
